...
parser.add_argument("--dump-user-agent", action='store_true',
                    help="display the current browser identification")
parser.add_argument(
    "--user-agent",
    nargs="?",
    default="Mozilla/5.0 (Windows NT 7.0; rv:25.0) Gecko/20100101 Firefox/25.0",
...
parser.add_argument("--get-pageurl", action='store_true',
                    help="simulate, quiet but print URL")
parser.add_argument("--get-title", action='store_true',
                    help="simulate, quiet but print title")
parser.add_argument("--get-posts", action='store_true',
                    help="simulate, quiet but print user posts")
parser.add_argument("--get-id", action='store_true',
                    help="simulate, quiet but print id")
parser.add_argument("--get-thumbnail", action='store_true',
                    help="simulate, quiet but print thumbnail URL")
parser.add_argument("--get-filename", action='store_true',
                    help="simulate, quiet but print output filename")
parser.add_argument("--get-format", action='store_true',
                    help="simulate, quiet but print file format")
parser.add_argument("--get-type", action='store_true',
                    help="simulate, quiet but print file type")
parser.add_argument("--get-username", action='store_true',
                    help="simulate, quiet but print uploaders username")
parser.add_argument("--get-bbcode", action='store_true',
                    help="simulate, quiet but print bbcode")
parser.add_argument("--get-html", action='store_true',
                    help="simulate, quiet but print html code")
parser.add_argument(
    "--get-dimensions",
    action='store_true',
    help="simulate, quiet but print dimensions (width x height)")
parser.add_argument("--get-width", action='store_true',
                    help="simulate, quiet but print width")
parser.add_argument("--get-height", action='store_true',
                    help="simulate, quiet but print height")
parser.add_argument("--get-views", action='store_true',
                    help="simulate, quiet but print number of views")
parser.add_argument("--get-favorites", action='store_true',
...
if (getargs.update):
    ...
    geturls_opener.addheaders = [
        ("Referer",
         "https://github.com/GameMaker2k/Python-Scripts/"),
        ("User-Agent",
         fakeua),
        ("Accept-Encoding",
         "gzip, deflate"),
    ...
    geturls_text = geturls_opener.open(
        "https://raw.github.com/GameMaker2k/Python-Scripts/master/MiniScripts/motherless-dl.py")
...
def motherless_dl(mtlessgetargs=vars(getargs)):
    ...
    while (cururlarg < numurlarg):
        ...
        if ((re.match("^galleries",
            ...
            out_text = re.sub(
                re.escape("https://motherless.com"), "", out_text)
            out_text = re.sub(
            ...
            while (curpage <= numpages):
                ...
                while (curgal < numgal):
                    ...
                    curgal = curgal + 1
                curpage = curpage + 1
        if (not re.match("^galleries",
                         mlessvidid[1]) or (re.match("^galleries",
                                                     mlessvidid[1]) and len(mlessvidid) < 4) or (re.match("^galleries",
                                                                                                          mlessvidid[1]) and len(mlessvidid) > 5)):
        ...
        while (curusrgal < numusrgal):
            ...
            if (
                (
                    re.match(
                        "^G",
                        mlessvidid[1]) and len(mlessvidid) == 2) or (
                    re.match(
                    "^H",
                    mlessvidid[1]) and len(mlessvidid) == 2) or (
                        re.match(
                            ...
                            mlessvidid[1]) and len(mlessvidid) == 2) or (
                                re.match(
                                    ...
                                    mlessvidid[1]) and len(mlessvidid) == 2) or (
                                        re.match(
                                            "^g",
                                            mlessvidid[1]) and len(mlessvidid) == 3) or (
                                                re.match(
                                                    "^u",
                                                    mlessvidid[1]) and len(mlessvidid) == 3) or (
                                                        re.match(
                                                            ...
                                                            mlessvidid[1]) and (
                                                                re.match(
                                                                    "^videos",
                                                                    mlessvidid[2]) or re.match(
                                                                        "^images",
                                                                        mlessvidid[2])) and len(mlessvidid) == 4) or (
                                                                            re.match(
                                                                                "^f",
                                                                                mlessvidid[1]) and len(mlessvidid) == 4 and (
                                                                                    re.match(
                                                                                        "^videos",
                                                                                        mlessvidid[3]) or re.match(
                                                                                            "^images",
                                                                                            mlessvidid[3]))) or (
                                                                                                re.match(
                                                                                                    "^live",
                                                                                                    mlessvidid[1]) and len(mlessvidid) == 3 and (
                                                                                                        re.match(
                                                                                                            "^images",
                                                                                                            mlessvidid[2]) or re.match(
                                                                                                                "^videos",
